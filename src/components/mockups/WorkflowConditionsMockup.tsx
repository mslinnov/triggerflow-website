  ...
  return (
    <div className="h-full w-full p-5 bg-gray-50">
      ...
      <div className="flex items-center gap-2 mb-4 px-2">
        ...
        <span className="text-[10px] text-gray-500">Workflow: <span className="font-medium text-gray-700">Offres personnalisees post-reservation</span></span>
      </div>

      {/* Incoming connector */}
      <div className="flex justify-center mb-2">
        <div className="flex flex-col items-center">
          <div className="text-[9px] text-gray-400 mb-1">... etapes precedentes</div>
      ...
      <div className="flex justify-center mb-3">
        <div className="bg-purple-50 border-2 border-purple-300 rounded-xl px-5 py-3 flex items-center gap-2.5 shadow-sm relative">
          ...
          <div>
            <p className="text-xs font-semibold text-purple-800">Type de sejour ?</p>
            <p className="text-[9px] text-purple-500">Variable: reservation.type_sejour</p>
          </div>
        </div>
      </div>
      ...
      <div className="flex justify-center mt-3">
        <div className="flex flex-col items-center">
          <div className="flex items-center gap-1">
            <div className="w-10 h-0.5 bg-pink-300" />
          ...
          <div className="bg-white border border-gray-200 rounded-lg px-3 py-1.5 mt-0.5">
            <p className="text-[9px] text-gray-500 text-center">Suite du workflow...</p>
          </div>
        </div>
      </div>
...
function BranchCard({
  ...
  return (
    <div className="flex flex-col items-center">
      ...
      {/* Condition label */}
      <span className={`text-[8px] font-semibold px-2 py-0.5 rounded-full mb-1 ${s.badge}`}>
        {isDefault ? 'Sinon' : `Si "${condition}"`}
      </span>
      <div className={`w-0.5 h-2 ${connectorColor}`} />
      {/* Action card */}
      <div className={`${s.bg} border ${s.border} rounded-xl p-2.5 text-center w-full`}>
        <div className={`w-7 h-7 rounded-lg ${s.iconBg} flex items-center justify-center mx-auto mb-1`}>
          <Icon className={`w-3.5 h-3.5 ${s.iconColor}`} />
        </div>
        <p className="text-[9px] font-semibold text-gray-800 leading-tight">{action}</p>
...
function StatBadge({ label, value, color }: { label: string; value: string; color: string }) {
  return (
    <div className={`flex items-center gap-1 px-2 py-0.5 rounded-full text-[9px] font-medium ${color}`}>
      <span>{label}</span>
