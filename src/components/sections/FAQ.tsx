        ...
        <div className="mx-auto mt-12 max-w-3xl space-y-4">
          {faqItems.map((item, index) => (
            <motion.div
              ...
              className="overflow-hidden rounded-xl border border-zinc-100 bg-zinc-50/50 shadow-sm"
            >
              <button
                onClick={() => toggle(index)}
                className="flex w-full items-center justify-between p-5 text-left"
              >
                <span className="pr-4 font-medium text-brand-dark">
              ...
              <AnimatePresence initial={false}>
                {openIndex === index && (
                  ...
                  >
                    <div className="border-t border-zinc-100 bg-white px-5 pb-5 pt-4 text-zinc-600">
                      {t(`items.${item}.answer`)}
                    </div>
                  </motion.div>
                )}
