...
| Advertising cookies (Meta pixel) | 13 months maximum |
...
| Advertising cookies | Meta pixel: measuring how our Facebook and Instagram ads perform, and excluding visitors we already know from our campaigns | Yes |

Audience measurement and advertising are two distinct purposes, subject to two distinct consents: accepting one never amounts to accepting the other. The Meta pixel is only loaded after the advertising category has been explicitly accepted; if it is declined or withdrawn, the `_fbp` and `_fbc` cookies are deleted.

The Meta pixel involves transmitting data to Meta Platforms Ireland Limited, acting as joint controller for that operation. A transfer to the United States may occur, framed by the Data Privacy Framework and the Standard Contractual Clauses.
